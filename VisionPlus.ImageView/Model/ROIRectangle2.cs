...
namespace Lxc.VisionPlus.ImageView.Model
{
    ...
    public HTuple cols;
        ...
        public void UpdateROI(double row, double col, double phi, double length1, double length2)
        {
            this.Row = col;
            this.Column = row;
            this.length1 = length1 / 2.0;
            this.length2 = length2 / 2.0;
        ...
        public override void createInitRectangle2(double imageHeight)
        {
            ...
            base.createRectangle2(Row, Column, phi, length1, length2);
        ...
        public override void draw(HalconDotNet.HWindow window, int imageWidth, int imageHeight)
		{
			...
			HXLDCont rectangle2xld = new HXLDCont();
			rectangle2xld.GenRectangle2ContourXld(Row, Column, phi, length1, length2);
			arrowxld = GenArrowContourXld(Row, Column, Row + (Math.Sin(-phi) * length1 * 1.2),
				Column + (Math.Cos(-phi) * length1 * 1.2), littleRecSize*10, littleRecSize*10);
			window.DispXld(rectangle2xld);
			window.SetDraw("fill");
			window.SetColor("blue");
			for (int i = 0; i < NumHandles; i++)
            {
				window.DispRectangle2(rows[i].D, cols[i].D, phi, littleRecSize + 5, littleRecSize + 5);
        ...
        public override void displayActive(HalconDotNet.HWindow window, int imageWidth, int imageHeight)
		{
            ...
            window.DispRectangle2(rows[activeHandleIdx].D,
                                  ...
                                  phi, littleRecSize + 5, littleRecSize + 5);

            if (activeHandleIdx == 5)
			{
				
				  HXLDCont arrowxld = new HXLDCont();
				arrowxld = GenArrowContourXld(Row, Column,Row + (Math.Sin(-phi) * length1 * 1.2),Column + Math.Cos(-phi) * length1 * 1.2, littleRecSize*10, littleRecSize*10);
				window.DispXld(arrowxld);
				arrowxld.Dispose();	
		...
		{
			return new HTuple(new double[] { Row, Column, phi, length1, length2 });
		}
		...
		public override void moveByHandle(double newX, double newY)
		{
			...
			switch (activeHandleIdx)
			{
				...
				case 5:
					...
					//HOperatorSet.TupleAtan2(vY, vX,out PHI);
		...
		private void updateHandlePos()
		{
			...
			hom2D = hom2D.HomMat2dTranslate(Column, Row);
