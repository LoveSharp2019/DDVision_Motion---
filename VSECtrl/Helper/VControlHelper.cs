...
namespace VControls.Helper
{
    ...
    /// Class ControlHelper.
    /// </summary>
    public static class VControlHelper
    {
        ...
        public static void SetControlEnabled(this Control c, bool enabled)
        {
            if (!c.IsDisposed)
            {
                if (enabled)
                {
                    VControlHelper.SetWindowLong(c.Handle, -16, -134217729 & VControlHelper.GetWindowLong(c.Handle, -16));
                }
                else
                {
                    VControlHelper.SetWindowLong(c.Handle, -16, 134217728 + VControlHelper.GetWindowLong(c.Handle, -16));
                }
            }
        }
            ...
            string strRegexPattern = null)
        {
            ...
            switch (inputType)
            {
                case VTextInputType.NotControl:
                    result = true;
                    return result;
                case VTextInputType.UnsignNumber:
                ...
                case VTextInputType.Integer:
                    ...
                    break;
                case VTextInputType.PositiveInteger:
                    if (string.IsNullOrEmpty(strValue))
                    {
                        result = true;
                        return result;
            ...
            {
                if (inputType == VTextInputType.Number || inputType == VTextInputType.UnsignNumber || inputType == VTextInputType.PositiveNumber)
                {
                    if (strValue.IndexOf(".") >= 0)
                    {
        ...
        public static void FreezeControl(Control control, bool blnToFreeze)
        {
            if (blnToFreeze && control.IsHandleCreated && control.Visible && !control.IsDisposed && (!m_lstFreezeControl.ContainsKey(control) || (m_lstFreezeControl.ContainsKey(control) && m_lstFreezeControl[control] == false)))
            {
                ...
                VNativeMethods.SendMessage(control.Handle, 11, 0, 0);
            }
            else if (!blnToFreeze && !control.IsDisposed && m_lstFreezeControl.ContainsKey(control) && m_lstFreezeControl[control] == true)
            {
                ...
                VNativeMethods.SendMessage(control.Handle, 11, 1, 0);
        ...
        public static void PaintTriangle(Graphics g, System.Drawing.Brush brush, Point point, int size, VGraphDirection direction)
        {
            Point[] array = new Point[4];
            switch (direction)
            {
                case VGraphDirection.Leftward:
                    array[0] = new Point(point.X, point.Y - size);
                    array[1] = new Point(point.X, point.Y + size);
                    array[2] = new Point(point.X - 2 * size, point.Y);
                    break;
                case VGraphDirection.Rightward:
                    array[0] = new Point(point.X, point.Y - size);
                    array[1] = new Point(point.X, point.Y + size);
                    array[2] = new Point(point.X + 2 * size, point.Y);
                    break;
                case VGraphDirection.Upward:
        ...
        /// <param name="size">The size.</param>
        /// <param name="direction">The direction.</param>
        public static void PaintTriangle(Graphics g, System.Drawing.Brush brush, PointF point, int size, VGraphDirection direction)
        {
...
#pragma warning disable CS0414 // 字段“VControlHelper.SB_CTL”已被赋值，但从未使用过它的值
        static uint SB_CTL = 0x2;
#pragma warning restore CS0414 // 字段“VControlHelper.SB_CTL”已被赋值，但从未使用过它的值
#pragma warning disable CS0414 // 字段“VControlHelper.SB_BOTH”已被赋值，但从未使用过它的值
        static uint SB_BOTH = 0x3;
#pragma warning restore CS0414 // 字段“VControlHelper.SB_BOTH”已被赋值，但从未使用过它的值
