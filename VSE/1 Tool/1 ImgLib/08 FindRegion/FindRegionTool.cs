...
{
    [Serializable]
    internal class FindRegionTool : ToolBase
    {
        internal FindRegionTool()
        {
        ...
        public override void Run(bool updateImage, bool runTool, string toolName)
        {
            try
            {
                lock (obj)
                {
                    ...
                    if (updateImage)
                    {
                        //Win_FindRegionTool.Instance.PImageWin.Image = toolPar.InputPar.图像;
                    }
                    ...
                    if (toolPar.RunPar.modeGray || toolPar.RunPar.modeAll)
                    {
                        toolPar.ResultsPar.resultGrayRegion = null;
                        toolPar.ResultsPar.resultRegion = null;
                        double deviation = 0;
                        double averageGray = image.Intensity(toolPar.ResultsPar.搜索区域.getRegion(), out deviation);
                        double dThresholdMin = 0;
                        double dThresholdMax = 0;
                        if (averageGray < toolPar.RunPar.dTorMin)
                        {
                            dThresholdMin = averageGray;
                            toolPar.RunPar.dTorMin = averageGray;
                        }
                        if (averageGray > toolPar.RunPar.dTorMax)
                        {
                            dThresholdMax = averageGray;
                        }
                        dThresholdMin = averageGray - toolPar.RunPar.dTorMin;
                        dThresholdMax = averageGray + toolPar.RunPar.dTorMax;
                        if (dThresholdMax > 255)
                        {
                            dThresholdMax = 255;
                        }
                        HRegion hRegion1 = image.ReduceDomain(toolPar.ResultsPar.搜索区域.getRegion()).Threshold((double)0, dThresholdMin);
                        HRegion hRegion2 = image.ReduceDomain(toolPar.ResultsPar.搜索区域.getRegion()).Threshold(dThresholdMax, (double)255);
                        toolPar.ResultsPar.resultGrayRegion = hRegion1.Union2(hRegion2);
                        if (toolPar.RunPar.modeGray && !toolPar.RunPar.modeAll)
                        {
                            toolPar.ResultsPar.resultRegion = toolPar.ResultsPar.resultGrayRegion;
                        }
                    }

                    if (toolPar.RunPar.modeContour || toolPar.RunPar.modeAll)
                    {
                        toolPar.ResultsPar.resultContourRegion = null;
                        toolPar.ResultsPar.resultRegion = null;
                        HImage hImage1 = image.ReduceDomain(toolPar.ResultsPar.搜索区域.getRegion());
                        HImage ImgTemp = new HImage();
                        hImage1.EdgesImage(out ImgTemp, "canny", toolPar.RunPar.dAlpha, "nms", toolPar.RunPar.dThresoldMin, toolPar.RunPar.dThresoldMax);
                        HRegion regionTemp = ImgTemp.Threshold(toolPar.RunPar.dThresoldMin, 255.0);
                        toolPar.ResultsPar.resultContourRegion = regionTemp;
                        if (toolPar.RunPar.modeContour && !toolPar.RunPar.modeAll)
                        {
                            toolPar.ResultsPar.resultRegion = toolPar.ResultsPar.resultContourRegion;
                        }
                    }

                    if (toolPar.RunPar.modeAll)
                    {
                        toolPar.ResultsPar.resultRegion = toolPar.ResultsPar.resultGrayRegion.Union2(toolPar.ResultsPar.resultContourRegion);
                    }
                    ...
                    HTuple hResultCount = 0;
                    hResultCount = resultRegion.CountObj();
                    if (hResultCount > 0)
                    {
                        toolPar.ResultsPar.Consequence = true;
                    }
                    else
                    {
                        toolPar.ResultsPar.Consequence = false;
                    }
                    ...
                    if (Win_FindRegionTool.Instance.Visible)
                    {
                        Win_FindRegionTool.Instance.LoadResult();
                        Win_FindRegionTool.Instance.PImageWin.Image = toolPar.InputPar.图像;
                        if (toolPar.RunPar.displayGoldenSearchRegion)
                        {
                            Win_FindRegionTool.Instance.PImageWin.displayHRegion(toolPar.RunPar.基准区域.getRegion(), "orange", "margin");
                        }
                        if (toolPar.RunPar.displaySearchRegion)
                        {
                            if (toolPar.ResultsPar.Consequence)
                            {
                                Win_FindRegionTool.Instance.PImageWin.displayHRegion(toolPar.ResultsPar.搜索区域.getRegion(), "blue", "margin");
                            }
                            else
                            {
                                Win_FindRegionTool.Instance.PImageWin.displayHRegion(toolPar.ResultsPar.搜索区域.getRegion(), "red", "margin");
                            }
                        }
                        if (toolPar.RunPar.displayResultRegion)
                        {
                            if (toolPar.ResultsPar.resultRegion != null)
                            {
                                Win_FindRegionTool.Instance.PImageWin.displayHRegion(toolPar.ResultsPar.resultRegion.Union1(), "green", toolPar.RunPar.regionDrawMode == FillMode.Margin ? "margin" : "fill");
                            }
                        }
                    }
                    ...
                    if (toolPar.RunPar.displaySearchRegion)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                GetImageWindowControl().BeginInvoke(new MethodInvoker(() =>
                                {
                                    GetImageWindowControl().displayHRegion(toolPar.ResultsPar.搜索区域.getRegion(), "red", "margin");
                                }));
                            }
                        }
                    }
                    ...
                    if (toolPar.RunPar.displayResultRegion)
                    {
                        if (toolPar.ResultsPar.resultRegion != null)
                        {
                            ...
                            else
                            {
                                GetImageWindowControl().BeginInvoke(new MethodInvoker(() =>
                                {
                                    GetImageWindowControl().displayHRegion(toolPar.ResultsPar.resultRegion.Union1(), "green", toolPar.RunPar.regionDrawMode == FillMode.Margin ? "margin" : "fill");
                                }));
                            }
                        }
                    }
                    ...
                    if ((toolPar.RunPar.displayOutCircle || toolPar.RunPar.displayCross) && count < 1000)
                    {
                        for (int i = 0; i < resultCount; i++)
                        {
                            ...
                            if (toolPar.RunPar.displayOutCircle)
                            {
                                ...
                                if (Win_FindRegionTool.Instance.Visible)
                                {
                                    Win_FindRegionTool.Instance.PImageWin.displayHRegion(circle, "green", toolPar.RunPar.outCircleDrawMode == FillMode.Margin ? "margin" : "fill");
                                }
                                else
                                {
                                    if (runTool)
        ...
        public class ToolPar : ToolParBase
        {
            ...
            private ResultsPar _resultPar = new ResultsPar();
        ...
        public class RunPar
        {
            ...
            internal bool modeAll = false;

            internal bool modeGray = true;

            internal double dTorMin = 10;

            internal double dTorMax = 10;

            internal double dAlpha = 1;

            internal double dThresoldMin = 10;

            internal double dThresoldMax = 20;

            internal double dContourArea = 100;

            internal bool modeContour = false;
        ...
        public class ResultsPar 
        {
            ...
            private HRegion _resultGrayRegion;
            internal HRegion resultGrayRegion
            {
                get { return _resultGrayRegion; }
                set { _resultGrayRegion = value; }
            }

            private HRegion _resultContourRegion;
            internal HRegion resultContourRegion
            {
                get { return _resultContourRegion; }
                set { _resultContourRegion = value; }
            }
            ...
            internal HRegion resultRegion
            {
                get { return _resultRegion; }
