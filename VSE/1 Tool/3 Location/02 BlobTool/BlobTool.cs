...
{
    [Serializable]
    internal class BlobTool : ToolBase
    {
        internal BlobTool()
        {
        ...
        public override void Run(bool updateImage, bool runTool, string toolName)
        {
            ...
            try
            {
                lock (obj)
                {
                    ...
                    if (updateImage)
                    {
                        //Win_BlobTool.Instance.PImageWin.Image = toolPar.InputPar.图像;
                    }
                    ...
                    resultRegion = image.ReduceDomain(toolPar.ResultsPar.搜索区域.getRegion()).Threshold((double)toolPar.RunPar.MinThreshold, (double)toolPar.RunPar.MaxThreshold);
                    ...
                    toolPar.ResultsPar.L_resultBlob.Clear();
                    ...
                    if (resultRegion.Row.Length != 0)
                    {
                        ...
                        for (int i = 0; i < s.GetUpperBound(0) + 1; i++)
                        {
                            ...
                            toolPar.ResultsPar.L_resultBlob.Add(br);
                        }
                    }
                    ...
                    if (Win_BlobTool.Instance.Visible)
                    {
                        Win_BlobTool.Instance.LoadResult();
                        Win_BlobTool.Instance.PImageWin.Image = toolPar.InputPar.图像;
                        if (toolPar.RunPar.displayGoldenSearchRegion)
                        {
                            Win_BlobTool.Instance.PImageWin.displayHRegion(toolPar.RunPar.基准区域.getRegion(), "orange", "margin");
                        }
                        if (toolPar.RunPar.displaySearchRegion)
                        {
                            Win_BlobTool.Instance.PImageWin.displayHRegion(toolPar.ResultsPar.搜索区域.getRegion(), "blue", "margin");
                        }
                        if (toolPar.RunPar.displayResultRegion)
                        {
                            if (toolPar.ResultsPar.resultRegion != null)
                            {
                                Win_BlobTool.Instance.PImageWin.displayHRegion(toolPar.ResultsPar.resultRegion.Union1(), "green", "fill");
                            }
                        }
                    }
                    ...
                    if (toolPar.RunPar.displayResultRegion)
                    {
                        if (runTool)
                        {
                            Win_BlobTool.Instance.PImageWin.displayHRegion(toolPar.ResultsPar.resultRegion.Union1(), "green", toolPar.RunPar.regionDrawMode == FillMode.Margin ? "margin" : "fill");
                        }
                        else
                        {
                            GetImageWindowControl().BeginInvoke(new MethodInvoker(() =>
                    ...
                    if ((toolPar.RunPar.displayOutCircle || toolPar.RunPar.displayCross) && count < 1000)
                    {
                        for (int i = 0; i < resultCount; i++)
                        {
                            ...
                            if (toolPar.RunPar.displayOutCircle)
                            {
                                ...
                                if (Win_BlobTool.Instance.Visible)
                                {
                                    Win_BlobTool.Instance.PImageWin.displayHRegion(circle, "green", toolPar.RunPar.outCircleDrawMode == FillMode.Margin ? "margin" : "fill");
                                }
                                else
                                {
                                    if (runTool)
                    ...
                    for (int i = 0; i < toolPar.ResultsPar.L_resultBlob.Count; i++)
                    {
                        XYU xyu = new XYU();
                        xyu.Point.X = toolPar.ResultsPar.L_resultBlob[i].CenterOfMassX;
                        xyu.Point.Y = toolPar.ResultsPar.L_resultBlob[i].CenterOfMassY;
                        xyu.U = 0;
        ...
        public class ResultsPar 
        {
            ...
            internal List<ResultPar> L_resultBlob = new List<ResultPar>();
        ...
        public class ResultPar 
        {
            ...
            //public ResultPar(ResultPar other)
            //{
            //    this.mBlobID = other.mBlobID;
            //    this.mFilteredOut = other.mFilteredOut;
            //    this.mFilteredResultOut = other.mFilteredResultOut;
            //}

            //public ResultPar()
            //{
            //    this.mBlobID = -1;
            //    this.mFilteredOut = true;
            //    this.mFilteredResultOut = new double[37];
            //}
